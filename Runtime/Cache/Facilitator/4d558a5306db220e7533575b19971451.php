...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('ProList/pyadd');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('ProList/py_changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">上 架</button>
            <button class="btn ajax-post confirm" url="<?php echo U('ProList/py_changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">下 架</button>
            <button class="btn ajax-post confirm" url="<?php echo U('ProList/py_changeStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
		<!-- 高级搜索 -->
        <div class="search-form fr cf">
			<div class="sleft">
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('ProList/pylist');?>"><i class="btn-search"></i></a>
			</div>
		</div>
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
				<a href="<?php echo U('ProList/pyedit?id='.$vo['id']);?>" target="_blank">编辑</a>
				<?php if($vo['status'] == '0'): ?><a href="<?php echo U('ProList/py_changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">上架</a>
				<?php elseif($vo['status'] == '1'): ?>
					<a href="<?php echo U('ProList/py_changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">下架</a><?php endif; ?>
				
				<a href="<?php echo U('ProList/py_changeStatus?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
    ...
    //导航高亮
    $('.side-sub-menu').find('a[href="<?php echo U('ProList/pylist');?>"]').closest('li').addClass('current');
	</script>
