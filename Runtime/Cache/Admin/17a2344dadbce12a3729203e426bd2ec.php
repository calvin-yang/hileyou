...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('product/lvadd');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('Product/lv_changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('Product/lv_changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Product/lv_delete',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<?php $produc_type = C('PRODUC_TYPE'); $room_type = C('ROOM_TYPE'); ?>
			<td><?php echo $produc_type[$vo['facilities_type']];?></td>
			<td>
				<?php if(($vo['facilities_type']) == "1"): echo $room_type[$vo['type']]; endif; ?>
				<?php if(($vo['facilities_type']) == "2"): ?>不限<?php endif; ?>
				<?php if(($vo['facilities_type']) == "3"): ?>不限<?php endif; ?>
			</td>
			<td><?php echo str_cut($vo['name'],40);?></td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td>
				<a href="<?php echo U('Product/lvedit?id='.$vo['id']);?>" target="_blank">编辑</a>
				<?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Product/lv_changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('Product/lv_changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">启用</a><?php endif; ?>
				<a href="<?php echo U('Product/lv_delete?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
    ...
    //导航高亮
    $('.side-sub-menu').find('a[href="<?php echo U('Product/lvlist');?>"]').closest('li').addClass('current');
	</script>
