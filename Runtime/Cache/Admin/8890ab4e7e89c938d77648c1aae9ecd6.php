...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('Product/eatadd');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('Product/eat_changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">上架</button>
            <button class="btn ajax-post" url="<?php echo U('Product/eat_changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">下架</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Product/eat_changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">通 过</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Product/eat_changeStatus',array('method'=>'falsestatus'));?>" target-form="ids">不 通 过</button>
            <button class="btn" url="" onclick="setDivCenter('0','#model')" target-form="ids">发送通知</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Product/eat_changeStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
		<!-- 高级搜索 -->
        <div class="search-form fr cf">
			<div class="sleft">
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('product/eatlist',false);?>"><i class="btn-search"></i></a>
			</div>
		</div>
    </div>
    <!-- 数据列表 -->
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
				<a href="<?php echo U('Product/eatedit?id='.$vo['id']);?>" target="_blank">编辑</a>
				<?php if($vo['status'] == '0'): ?><a href="<?php echo U('Product/eat_changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">上架</a>
				<?php elseif($vo['status'] == '1'): ?>
					<a href="<?php echo U('Product/eat_changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">下架</a>
				<?php elseif($vo['status'] == '3'): ?>
					<a href="<?php echo U('Product/eat_changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">通过</a>

					<a href="<?php echo U('Product/eat_changeStatus?method=falsestatus&id='.$vo['id']);?>" class="ajax-get">不通过</a>
				<?php elseif($vo['status'] == '4'): ?>
					<a href="<?php echo U('Product/eat_changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">通过</a><?php endif; ?>
				<a href="javascript:;" onclick="setDivCenter(<?php echo $vo['uid'];?>,'#model')">发送通知</a>
				<a href="<?php echo U('Product/eat_changeStatus?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
