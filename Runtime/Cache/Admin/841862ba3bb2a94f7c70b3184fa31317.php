...
<body>
	...
	<div class="cf">
		<div class="fl">
            <button class="btn">平台总金额：<?php echo $price_num;?>&nbsp; RMB</button>
        </div>
    ...
    <thead>
		...
		<th class="">用户ID</th>
		<th class="">用户拥有金额(￥)</th>
		<th class="">最后一笔交易金额(￥)</th>
		<th class="">最后一笔交易世间</th>
		<th class="">注册手机号</th>
		<th class="">用户真实姓名</th>
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo time_format($vo['last_time']);?></td>
			<td><?php echo $vo['mobile'];?></td>
			<td><?php echo $vo['realname']?$vo['realname']:'暂无';?></td>
    ...
    //导航高亮
    $('.side-sub-menu').find('a[href="<?php echo U('Order/financial');?>"]').closest('li').addClass('current');
	</script>
