...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('Product/readd');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('Product/re_changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">上 架</button>
            <button class="btn ajax-post" url="<?php echo U('Product/re_changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">下 架</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Product/re_changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">通 过</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Product/re_changeStatus',array('method'=>'falsestatus'));?>" target-form="ids">不 通 过</button>
            <button class="btn" url="" onclick="setDivCenter('0','#model')" target-form="ids">发送通知</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Product/re_changeStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
        ...
        <div class="search-form fr cf">
			<div class="sleft">
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('product/relist',false);?>"><i class="btn-search"></i></a>
			</div>
		</div>
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo ((isset($vo["day"]) && ($vo["day"] !== ""))?($vo["day"]):''); ?>天<?php echo ((isset($vo["night"]) && ($vo["night"] !== ""))?($vo["night"]):''); ?>夜</td>
			<td><?php echo ($vo["team_number"]); ?></td>
			<td><?php echo (ceil($vo['favorable_price']) == 0)?$vo['price']:$vo['favorable_price'].'<br /><span style="color:darkgray;"><s>'.$vo['price'].'</s></span>';?></td>
			<td><?php echo ($vo["chief"]); ?></td>
			<td><?php echo ($vo["tel"]); ?></td>
			<td><?php echo time_format($vo.insert_time);?></td>
			<td><?php echo ($vo['nickname'] == '')?$vo['realname']:$vo['nickname'];?></td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td>
				<a href="<?php echo U('Product/reedit?id='.$vo['id'].'&type='.$vo['type']);?>" target="_blank">路线</a>
				<a href="<?php echo U('Product/re_pro_edit?id='.$vo['id'].'&night='.$vo['night'].'&day='.$vo['day']);?>" target="_blank">活动</a>
				<?php if($vo['status'] == '0'): ?><a href="<?php echo U('Product/re_changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">上架</a>
				<?php elseif($vo['status'] == '1'): ?>
					<a href="<?php echo U('Product/re_changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">下架</a>
				<?php elseif($vo['status'] == '3'): ?>
					<a href="<?php echo U('Product/re_changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">通过</a>

					<a href="<?php echo U('Product/re_changeStatus?method=falsestatus&id='.$vo['id']);?>" class="ajax-get">不通过</a>
				<?php elseif($vo['status'] == '4'): ?>
					<a href="<?php echo U('Product/re_changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">通过</a><?php endif; ?>
				<a href="javascript:;" onclick="setDivCenter(<?php echo $vo['uid'];?>,'#model')">发送通知</a>
				<a href="<?php echo U('Product/re_changeStatus?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
