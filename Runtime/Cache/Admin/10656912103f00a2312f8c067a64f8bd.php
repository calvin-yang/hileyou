...
<body>
    ...
    <div class="main-title">
        <h2>编辑线路活动&nbsp;&nbsp;<span class="check-tips"><?php echo I('get.day');?>天<?php echo I('get.night');?>夜游</span></h2>
    </div>
    
    <form action="/index.php?s=/product/re_pro_edit/id/12/night/6/day/7.html" method="post" class="form-horizontal">
        

        <div class="container-fluid">
            
            <?php $__FOR_START_15579__=1;$__FOR_END_15579__=I('get.night')+1;for($i=$__FOR_START_15579__;$i < $__FOR_END_15579__;$i+=1){ ?><div class="o">
            <div class="day-<?php echo ($i); ?>" style="margin-bottom:15px;">

                <div>
                    第<?php echo ($i); ?>天&nbsp;&nbsp;
                    <button class="btn submit-btn" id="insert" type="button" target-form="form-horizontal" onclick="insert_pro(this,<?php echo $i;?>);">添加活动</button>
                </div>
                <input type="hidden" name="day[]" value="<?php echo $i;?>" />
                <input type="hidden" name="route_id" value="<?php echo I('get.id');?>" />
                <?php $nu_number = 0; ?>
                
                <?php $table = 'RouteDetails'; $map['route_id'] = array('eq',I('get.id')); $map['day'] = array('eq',$i); $map['status'] = array('neq','2'); $list = get_table_fields($table,'',$map); ?>

                <?php if(is_array($list)): foreach($list as $key=>$vo): ?><input type="hidden" name="id<?php echo ($i); ?>[]" value="<?php echo ($vo["id"]); ?>" />
                    <div class="des">

                    <div class="pro-<?php echo ($i); ?> de">
                        <div class="form-item cf">
                            <label class="item-label">活动时间<span class="check-tips" csid="<?php echo ($vo["id"]); ?>" onclick="delete_hou(this);">(删除活动)</span></label>
                            <div class="controls">
                                <input type="time" class="text input-large " style="width:100px" name="start_time<?php echo ($i); ?>[]" value="<?php echo ($vo["start_time"]); ?>">
                                -
                                <input type="time" class="text input-large " style="width:100px" name="end_time<?php echo ($i); ?>[]" value="<?php echo ($vo["end_time"]); ?>">&nbsp;&nbsp;*
                            </div>
                        </div>
                        ...
                        <div class="form-item cf">
                            ...
                            <div class="controls">
                                <label class="textarea input-large">
                                    <textarea name="bewrite<?php echo ($i); ?>[]"><?php echo ($vo["bewrite"]); ?></textarea>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="form-item cf">
                            ...
                            <div class="">
                                <select style="width: 300px" class="chzn-select" id="type<?php echo ($nu_number); ?>" name="type<?php echo ($i); ?>[]" onchange="pro_select(this,<?php echo ($i); ?>);">
                                    <option value="">--请选择--</option>
                                    <option value="1" <?php if(($vo['type']) == "1"): ?>selected<?php endif; ?>>住</option>
                                    <option value="2" <?php if(($vo['type']) == "2"): ?>selected<?php endif; ?>>吃</option>
                                    <option value="3" <?php if(($vo['type']) == "3"): ?>selected<?php endif; ?>>玩</option>
                                    <option value="4" <?php if(($vo['type']) == "4"): ?>selected<?php endif; ?>>其他</option>
                                </select>&nbsp;&nbsp;*
                            </div>
                        </div>

                        <div class="form-item cf pro">
                            <?php if($vo['pro_id'] != ''): ?><span class="remov-<?php echo ($nu_number); ?>">
                                <label class="item-label">活动<span class="check-tips"></span></label>
                                <div class="">
                                    <?php $pro_list = get_pro_fields($vo['pro_id'],'id',$vo['type']); ?>
                                    <select style="width: 300px" class="chzn-select" id="pro_id" name="pro_id<?php echo ($i); ?>[]">
                                        <?php if(is_array($pro_list)): foreach($pro_list as $key=>$pro_list): ?><option value="<?php echo ($pro_list["id"]); ?>-<?php echo ($pro_list["name"]); ?>" <?php if(($vo['pro_id']) == $pro_list['id']): ?>selected<?php endif; ?>><?php echo ($pro_list["name"]); ?></option><?php endforeach; endif; ?>
                                    </select>
                                </div>
                            </span>&nbsp;&nbsp;*
                        </div><?php endif; ?>
                        <input type="hidden" name="pro_name<?php echo ($i); ?>[]" id="pro_name<?php echo ($i); ?>" />
                    </div>
                    <?php $nu_number = $nu_number + 1; ?>
                </div><?php endforeach; endif; ?>
                <input type="hidden" class="nu<?php echo $i;?>" value="<?php echo $nu_number;?>" />
