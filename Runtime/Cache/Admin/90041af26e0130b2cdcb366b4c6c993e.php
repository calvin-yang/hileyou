...
<body>
	...
	<div class="main-title">
		<h2><?php echo I('get.sort')?'子':'父';?>级分类</h2>
	</div>
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('Home/news_sort_add');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('Home/infor_changeStatus',array('method'=>'forbidUser','type' => 'nt'));?>" target-form="ids">禁用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Home/infor_changeStatus',array('method'=>'resumeuser','type' => 'nt'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Home/infor_changeStatus',array('method'=>'deleteUser','type' => 'nt'));?>" target-form="ids">删 除</button>
        </div>
    </div>
    <!-- 数据列表 -->
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('news_sort',array('pid' => $vo['id'],'sort' => ($vo['sort_type'] + 1)));?>" class="confirm"><?php echo ($vo["name"]); ?> </a></td>
			<td><a href="<?php echo U('news_sort',array('pid' => $vo['id'],'sort' => ($vo['sort_type'] + 1)));?>" class="confirm">请点击</a></td>
			<td><?php if(($vo['sort_type']) == "1"): ?>父级<?php else: ?>子级<?php endif; ?></td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td>
				<a href="<?php echo U('Home/news_sort_add?id='.$vo['id']);?>">编辑</a>
				<?php if(($vo['status']) == "0"): ?><a href="<?php echo U('Home/infor_changeStatus?method=resumeuser&type=nt&id='.$vo['id']);?>" class="confirm ajax-get">启用</a>
				<?php else: ?>
					<a href="<?php echo U('Home/infor_changeStatus?method=forbidUser&type=nt&id='.$vo['id']);?>" class="confirm ajax-get">禁用</a><?php endif; ?>
				<a href="<?php echo U('Home/infor_changeStatus?method=deleteUser&type=nt&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
	...
	<script type="text/javascript">
    //导航高亮
    $('.side-sub-menu').find('a[href="<?php echo U('home/news_sort');?>"]').closest('li').addClass('current');
	</script>
