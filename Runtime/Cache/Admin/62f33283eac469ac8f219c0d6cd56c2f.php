...
<body>
	...
	<div class="cf">
		<div class="fl">
            <!--<a class="btn" href="<?php echo U('Order/ordadd');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('Order/ord_changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('Order/ord_changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>-->
            <button class="btn ajax-post confirm" url="<?php echo U('Order/ord_changeStatus',array('method'=>'refund'));?>" target-form="ids">退 费</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Order/ord_changeStatus',array('method'=>'finish'));?>" target-form="ids">完 成</button>
		...
		<div class="search-form fr cf">
			<div class="sleft">
				<div class="drop-down">
					<span id="sch-sort-txt" class="sort-txt" data="<?php echo $_GET['status'];?>"><?php if($_GET['status'] == ''): ?>所有<?php else: echo show_order_status_op($_GET['status']); endif; ?></span>
				...
				<input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="请输入订单号">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Order/ordlist',false);?>"><i class="btn-search"></i></a>
			</div>
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo (date('Y-m-d',$vo["insert_time"])); ?></td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td>
				<a href="<?php echo U('ord_detail',array('pro_order_id'=>$vo['id']));?>">详情</a>
				<?php if(($vo['status']) == "5"): ?><a href="<?php echo U('Order/ord_changeStatus?method=fsrefund&id='.$vo['order_id']);?>" class="confirm ajax-get">撤销</a><?php endif; ?>
				<?php if($vo['status'] == 7 || $vo['status'] == 5): ?><a href="<?php echo U('Order/ord_changeStatus?method=refund&id='.$vo['order_id']);?>" class="confirm ajax-get">退费</a><?php endif; ?>
				<?php if(($vo['status']) == "7"): ?><a href="<?php echo U('Order/ord_changeStatus?method=finish&id='.$vo['order_id']);?>" class="confirm ajax-get">完成</a><?php endif; ?>
				<?php if(($vo['status']) == "8"): ?><a href="<?php echo U('Order/ord_changeStatus?method=finan&id='.$vo['order_id']);?>" class="confirm ajax-get">提现完成</a><?php endif; ?>
				<a href="<?php echo U('Order/ord_changeStatus?method=deleteUser&id='.$vo['order_id']);?>" class="confirm ajax-get">删除</a>
            </td>
