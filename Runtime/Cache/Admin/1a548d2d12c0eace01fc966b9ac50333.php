...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('vip/user_add');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('vip/user_changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('vip/user_changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn" url="" onclick="setDivCenter('0','#model')" target-form="ids">发送通知</button>
            <button class="btn ajax-post confirm" url="<?php echo U('vip/user_delete',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>

        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="mobile" class="search-input" value="<?php echo I('mobile');?>" placeholder="请输入用户手机号">
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo time_format($vo['start_date'],'Y-m-d');?> 至 <?php echo time_format($vo['end_date'],'Y-m-d');?></td>
			<td><?php echo time_compare($vo['end_date']);?></td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td>
				<a href="<?php echo U('Vip/user_edit?id='.$vo['id']);?>" target="_blank">编辑</a>
				<?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('vip/user_changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('vip/user_changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">启用</a><?php endif; ?>
				<a href="<?php echo U('vip/user_delete?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
    ...
    //导航高亮
    $('.side-sub-menu').find('a[href="<?php echo U('vip/user');?>"]').closest('li').addClass('current');
	</script>
