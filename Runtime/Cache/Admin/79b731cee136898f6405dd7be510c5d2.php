...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('vip/add');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('vip/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('vip/changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('vip/delete',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo ($vo["monthly_price"]); ?></td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td>
				<a href="<?php echo U('Vip/edit?id='.$vo['id']);?>" target="_blank">编辑</a>
				<?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Vip/changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('Vip/changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">启用</a><?php endif; ?>
				<a href="<?php echo U('vip/delete?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
    ...
    //导航高亮
    $('.side-sub-menu').find('a[href="<?php echo U('vip/category');?>"]').closest('li').addClass('current');
	</script>
