...
<body>
	...
	<div class="cf">
		<div class="fl">
            ...
            <a class="btn" href="<?php echo U('facilitator/add');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('facilitator/changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post" url="<?php echo U('facilitator/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('facilitator/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">通 过</button>
            <button class="btn ajax-post confirm" url="<?php echo U('facilitator/changeStatus',array('method'=>'falsestatus'));?>" target-form="ids">不 通 过</button>
             <button class="btn" url="" onclick="setDivCenter('0','#model')" target-form="ids">发送通知</button>
            <button class="btn ajax-post confirm" url="<?php echo U('facilitator/delete',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>

        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="nickname" class="search-input" value="<?php echo I('nickname');?>" placeholder="请输入用户手机号/姓名">
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
				<?php if(($vo['type']) == "0"): ?>个人<?php else: ?>企业<?php endif; ?>
			</td>
			...
			<td>
				...
				<a href="<?php echo U('Facilitator/edit?id='.$vo['id']);?>" target="_blank">编辑&nbsp;&nbsp;&nbsp;</a>
				<?php if($vo['status'] == '0'): ?><a href="<?php echo U('Facilitator/changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">启用</a>
				<?php elseif($vo['status'] == '1'): ?>
					<a href="<?php echo U('Facilitator/changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php elseif($vo['status'] == '3'): ?>
					<a href="<?php echo U('Facilitator/changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">通过</a>

					<a href="<?php echo U('Facilitator/changeStatus?method=falsestatus&id='.$vo['id']);?>" class="ajax-get">不通过</a>

				<?php elseif($vo['status'] == '4'): ?>
					<a href="<?php echo U('Facilitator/changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">通过</a><?php endif; ?>

				<a href="<?php echo U('Facilitator/delete?id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
	    ...
	    <div id="model" class="model">  
	        ...
	        <div class="cento">
	        	<form action="<?php echo U('sendMessgae');?>" method="post" class="form-horizontal" enctype="multipart/form-data">
			        ...
			        <!-- <div class="form-item cf">
			            <label class="item-label">状态说明<span class="check-tips"></span></label>
			            <div class="controls">
			                <div class="radio">
			                  <label>
			                    <input type="radio" name="pro_status" id="pro_status" value="0" checked>
			                    &nbsp;禁 用
			                  </label>&nbsp;&nbsp;
			                  <label>
			                    <input type="radio" name="pro_status" id="pro_status" value="1" >
			                    &nbsp;启 用
			                  </label>&nbsp;&nbsp;
			                  <label>
			                    <input type="radio" name="pro_status" id="pro_status" value="2" >
			                    &nbsp;通 过
			                  </label>&nbsp;&nbsp;
			                  <label>
			                    <input type="radio" name="pro_status" id="pro_status" value="4" >
			                    &nbsp;不通过
			                  </label>&nbsp;&nbsp;
