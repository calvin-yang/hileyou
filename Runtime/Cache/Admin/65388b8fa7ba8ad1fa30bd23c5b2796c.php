...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('Home/inforadd');?>">新 增</a>
            <button class="btn ajax-post confirm" url="<?php echo U('Home/infor_changeStatus',array('method'=>'resumeuser'));?>" target-form="ids">发 布</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Home/infor_changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Home/infor_changeStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
    </div>
    <!-- 数据列表 -->
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo ($vo["news_sort_name"]); ?></td>
			<td><?php echo ($vo["publisher"]); ?></td>
			<td><?php echo (date('Y-m-d',$vo["insert_time"])); ?></td>
			<td><span style="cursor:pointer;" stickval = "<?php echo ($vo['stick']); ?>" onclick="return stick(this,<?php echo ($vo['id']); ?>)"><?php if(($vo['stick']) == "1"): ?>是<?php else: ?>否<?php endif; ?></span></td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td>
				<a href="<?php echo U('Home/inforedit?id='.$vo['id']);?>" target="_blank">编辑</a>
				<?php if(($vo['status']) == "0"): ?><a href="<?php echo U('Home/infor_changeStatus?method=resumeuser&id='.$vo['id']);?>" class="confirm ajax-get">发布</a>
				<?php else: ?>
					<a href="<?php echo U('Home/infor_changeStatus?method=forbidUser&id='.$vo['id']);?>" class="confirm ajax-get">禁用</a><?php endif; ?>
				
				<a href="<?php echo U('Home/infor_changeStatus?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
