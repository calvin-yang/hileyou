...
<body>
	...
	<div class="cf">
		<div class="fl">
            <button class="btn ajax-post confirm" url="<?php echo U('Points/usdelete',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
				...
				<a href="<?php echo U('Points/usdelete?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
    ...
    //导航高亮
    $('.side-sub-menu').find('a[href="<?php echo U('points/uslist');?>"]').closest('li').addClass('current');
	</script>
