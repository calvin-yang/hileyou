...
<body>
    ...
    <div class="main-title">
        <h2><?php echo ($info['id']?'编辑':'新增'); ?>文章分类</h2>
    </div>
    
    <form action="/index.php?s=/home/news_sort_add/id/2.html" method="post" class="form-horizontal">
            ...
            <div class="controls">
               <select class="chzn-select" id="pid" name="pid">
	               <option value="">父级分类</option>
                    <?php if($sort_list != false): if(is_array($sort_list)): $i = 0; $__LIST__ = $sort_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(($info['id']) == $vo['id']): ?>selected<?php endif; ?>>┯<?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        <?php if($vo['sorts'] != false): if(is_array($vo['sorts'])): $i = 0; $__LIST__ = $vo['sorts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vos): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vos["id"]); ?>" <?php if(($info['id']) == $vos['id']): ?>selected<?php endif; ?>>└<?php echo ($vos["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; endif; ?>
               </select>
            </div>
        </div>
