...
*$text_body = false;//纯文件内容
*$from_name	="tw";//发信人名称
*$to_name		="my";//收信人名称
*$from_mail	="service@twbbs.net.tw";//发信人邮件地址
*$to_mail 	= "tazen_007@56.com";//收信人邮件地址
*$subject 	= "test";//邮件标题
...
class MailSmtp { 
	...
	var $targetCode = "UTF-8";//默认邮件编码格式
/*========================================
*邮件服务器初始化
=========================================*/	
	Function MailSmtp($server="192.168.1.142",$port=25,$time_out=20) 	{
...
=========================================*/		
	function send($from_name,$to_name,$from_mail,$to_mail,$subject,$text_body=false,$html_body=false,$att=false) { 
		...
		$this->targetCode = $this->getUserEmailCodeType($to_mail);
		##转编码
		$text_body 	= $text_body;
		$html_body 	= $html_body;
		//$to_name 		= iconv("gbk",$this->targetCode,$to_name);
		//$subject 		= iconv("gbk",$this->targetCode,$subject);
	...
	##根据会员邮箱取得编码格式
  function getUserEmailCodeType($email){
          $emailEnd = strtolower(substr($email,strpos($email,"@")+1));
          switch($emailEnd){
                  case "gmail.com":
                  case "hotmail.com":
                  case "hotmail.com.tw":
