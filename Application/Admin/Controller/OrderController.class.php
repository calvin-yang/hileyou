...
class OrderController extends AdminController {
	public function index(){
		// $model = M('addons');
  //       $info = $model->where('name = "SocialComment"')->getField('config');
  //       $co = json_decode($info);
  //       dump($co);
       // $co->title = '海乐游开发团队';
        //$data['config'] = json_encode($co);
        //$model->where('name = "DevTeam"')->save($data);
  ...
  //       $this->display();
	...
	public function ord_changeStatus($method=null){
        ...
        switch ( strtolower($method) ){
            case 'finish':
                ...
                if($result){
                    ...
                    for ($i=0; $i < count($ids); $i++) { 
                        ...
                        #订单信息
                        $order_info = M('pro_order')->field('a.pro_id,a.uid,a.pro_type,a.discount_price,a.voucher_number,a.voucher_price,b.number order_number')->alias('a')->join(C('DB_PREFIX').'order b on a.id = b.pro_order_id')->where($map)->find();
                        #获取商家id
                        ...
                        #储存出入资金日志
                        $log_data[$i]['uid'] = $uid;
                        $log_data[$i]['order_id'] = $ids[$i];
                        $log_data[$i]['price'] = $price;
                        $log_data[$i]['type'] = 1;
                        $log_data[$i]['insert_time'] = time();
                        $log_data[$i]['content'] = '订单('.$order_info['order_number'].')已完成，用户ID为'.$order_info['uid'].'给商家用户ID为'.$uid.'支付金额￥'.$price;

                    }
                    M('finance_log')->addAll($log_data);
                }
                // $this->forbid('order', array('id'=>array('in',$id)) );
                break;
            case 'refund':
                $map['id'] = array('in',$id);
                $map['status'] = array('in','7,5');
                $data['status'] = 6;
                $list = $model->where($map)->select();
                if(count($list) != count($ids))
            ...
            case 'finan':
                $maps['id'] = array('in',$id);
                $maps['status'] = array('in','8');
                $data['status'] = 9;
                $list = $model->where($maps)->select();
                if(count($list) != count($ids))
                    $this->error('您只能选择(提现中)的订单进行操作');
                $result = $model->where($maps)->save($data);
    ...
    public function financial(){
        $model = M('financial');
        $list = $model->alias('a')->field('a.price,a.last_price,a.last_time,a.uid,b.mobile,b.email,c.nickname,c.realname')->join(C('DB_PREFIX').'ucenter_member b on a.uid = b.id')->join(C('DB_PREFIX').'member c on a.uid = c.uid')->select();
        #平台总金额
